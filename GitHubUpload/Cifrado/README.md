...
El instalador está definido en `installer.iss`. Para preparar una entrega, genera primero `AppLocker.exe` y ejecuta `make_release.ps1`; el script compila `AppLocker-Setup-<versión>.exe`, calcula su SHA-256 y crea `update.json` automáticamente. Solo falta subir esos dos archivos a tu servidor.
